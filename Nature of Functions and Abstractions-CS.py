...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        place_value = (1/7) % (0.1**(score - 1))
        digit = place_value // 0.1**(score)
        return int(digit)
    elif score >=7:
        sequence_score = 7 + (score % 6)
        place_value = (1/7) % (0.1**(sequence_score - 2))
        digit = place_value // (0.1**(sequence_score - 1))
        return int(digit)
    # END PROBLEM 2
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def statement (score0, score1, last_score=last_score, running_high=running_high):
        if who==0:
            if score0 - last_score> running_high :
                    running_high = score0 - last_score
                    print (running_high, "point(s)! That's a record gain for Player %s!"%(who))
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    dice_holder = 0
    while num_dice <= 10:
        current_average = averaged_dice(num_dice, dice)
        if current_average > dice_holder:
            dice_holder = current_average
            top_dice = num_dice
        num_dice += 1
    return top_dice
    # END PROBLEM 9
