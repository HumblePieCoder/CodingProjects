...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_checker = self.place
        place_count = 0
        while not place_checker.is_hive:
            if place_checker.bees and (self.min_range <= place_count <= self.max_range):
                return random_bee(place_checker.bees)
            place_checker = place_checker.entrance
            place_count += 1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.chew_countdown > 0:
            ...
            if self.place.bees != []:
                hungry_bee = random_bee(self.place.bees)
                hungry_bee.reduce_health(hungry_bee.health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        tank_ant_bees = list(self.place.bees)
        for bee in tank_ant_bees:
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that throws leaves at Bees underwater."""
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.is_QueenAnt:
            gamestate.is_QueenAnt = True
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        place_checker = self.place.exit
        while place_checker:
            if place_checker.ant:
                if place_checker.ant.is_container and place_checker.ant.ant_contained:
                    place_checker.ant.ant_contained.buff()
                place_checker.ant.buff()
            place_checker = place_checker.exit
        # END Problem 12
