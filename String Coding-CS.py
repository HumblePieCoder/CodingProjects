...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    filtered_paragraph = [string for string in paragraphs if select(string)]
    if k >= len(filtered_paragraph):
        return ''
    else:
        return filtered_paragraph[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    def about_checker(paragraph):
        string_paragraph = split(lower(remove_punctuation(paragraph)))
        lst = []
        for key in topic:
            for word in string_paragraph:
                if word == key:
                    lst = lst + [word] 
        return lst != []
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    min_valid_word = min(valid_words, key = lambda valid_word: diff_function(valid_word, typed_word, limit))
    if diff_function(min_valid_word, typed_word, limit) > limit:
        return typed_word
    else:
...
def feline_flips(start, goal, limit):
    ...
    if start[0] == goal[0]:
        return feline_flips(start[1:], goal[1:], limit) + 0
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i] and correct == True:
            sum_correct += 1
        else:
            correct = False
    ratio_correct = sum_correct / len(prompt)
    upload({'id': user_id, 'progress': ratio_correct})
...
def time_per_word(words, times_per_player):
    ...
    def difference(lst):
        if len(lst) == 1:
            ...
            return [lst[1] - lst[0]] + difference(lst[1:])    
    times_list = []
    for times in range(len(times_per_player)):
        times_list += [difference(times_per_player[times])]
...
def fastest_words(match):
    ...
    for word in word_indices:
        fastest_player = 0
        time_holder = time(match, 0, word)
        for player in player_indices:
            if time(match, player, word) < time_holder:
                fastest_player = player
                time_holder = time(match, player, word)
        player_words[fastest_player] += [word_at(match, word)]
    return player_words
    # END PROBLEM 10
